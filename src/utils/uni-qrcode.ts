// 二维码生成，修改自 https://github.com/dingyong0214/ThorUI-uniapp/blob/master/libs/weapp-qrcode.js
// 兼容 vue3
...
class QR8bitByte {
  data: any
  parsedData: any[] = []
  mode: any = QRMode.MODE_8BIT_BYTE
...
class QRUtil {
  ...
  static getMask(maskPattern: any, i: any, j: any) {
    switch (maskPattern) {
  ...
  static getLengthInBits(mode: any, type: number) {
    if (type >= 1 && type < 10) {
      switch (mode) {
...
class QRCode {
  private _htOption: Record<string, any>
  canvasId: string
  private _oQRCode: any
  static CorrectLevel = QRErrorCorrectLevel

  constructor(canvasId: string, vOption: any) {
    this._htOption = {
      width: 256,
      height: 256,
      typeNumber: 4,
  ...
  // 保存为图片，将临时路径传给回调
  exportImage(callback: CallableFunction, failFn?: (err: string) => void) {
    if (!callback)
      return
    ...
    uni.canvasToTempFilePath({
      ...
      fail: (err) => {
        failFn && failFn(err.errMsg ? JSON.stringify(err.errMsg) : JSON.stringify(err))
        console.error(err.errMsg || JSON.stringify(err))
      },
    })
  }
}
...
/** 生成二维码工具函数 */
export function generateQrCode(text: string, canvasId: string, qrCodeImageUrl: Ref<string>, qrCodeSize = 520) {
  uni.showLoading({})
